//
//  PlaceOrderListUnupController.m
//  离线app
//
...
#import "PlaceUpModel.h"
#import "SVProgressHUD.h"
@interface PlaceOrderListUnupController ()<UITableViewDelegate,UITableViewDataSource>
@property(nonatomic,strong)RootTableView *tableView;
@property(nonatomic,strong)NSArray *dataSource;
@property(nonatomic,strong)NSMutableDictionary *selectArr;
@end

@implementation PlaceOrderListUnupController
...
- (void)viewDidLoad {
    [super viewDidLoad];
    _selectArr = [[NSMutableDictionary alloc]init];
    [self searchDatabase];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if ([[_selectArr valueForKey:[NSString stringWithFormat:@"%ld",indexPath.section]] isEqualToString:@"1"]) {
        cell.headerImage.image = [UIImage imageNamed:@"ZJNativeBundle.bundle/icon_select_circle"];
    }else{
        cell.headerImage.image = [UIImage imageNamed:@"ZJNativeBundle.bundle/icon_unselect_circle"];
    }
    return cell;
}
-(void)selectAction:(UIButton *)sender{
    ...
    PlaceOrderListCell *cell = (PlaceOrderListCell *)obj;
    NSIndexPath *indexPath = [_tableView indexPathForCell:cell];
    if ([[_selectArr valueForKey:[NSString stringWithFormat:@"%ld",indexPath.section]] isEqualToString:@"1"]) {
        [_selectArr removeObjectForKey:[NSString stringWithFormat:@"%ld",indexPath.section]];
    }else{
        [_selectArr setValue:@"1" forKey:[NSString stringWithFormat:@"%ld",indexPath.section]];
    }
...
-(void)saveAction{
    NSArray *keys = [_selectArr allKeys];
    if (keys.count == 0) {
        [SVProgressHUD showImage:nil status:@"请至少选择一个订单"];
        return;
    }
    NSMutableString *string = [[NSMutableString alloc]initWithString:@"["];
    for (NSString *indexStr in keys) {
        PlaceOrderModel *model = [_dataSource objectAtIndex:indexStr.integerValue];
        PlaceUpModel *modelUp = [[PlaceUpModel alloc]init];
        modelUp.date = model.date;
        modelUp.customer = model.customer;
        modelUp.total = model.total;
        modelUp.staff = model.staff;
        modelUp.company = model.company;
        modelUp.id = model.id;
        
        modelUp.goods = [(NSMutableArray <PlaceUpGoodModel>*)[NSMutableArray alloc]init];
        NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"PlaceOrderGoodModel"];
        ...
        for (PlaceOrderGoodModel *model1 in arr) {
            PlaceUpGoodModel *model2 = [[PlaceUpGoodModel alloc]init];
            model2.materialCode = model1.materialCode;
            model2.salesID = model1.salesID;
            model2.type = model1.salesType;
            model2.bigunit = model1.bigunit;
            model2.midunit = model1.midunit;
            model2.smallunit = model1.smallunit;
            model2.quantity = model1.quantity;
            ...
            model2.amount = model1.amount;
            model2.note = model1.note;
            [modelUp.goods addObject:model2];
        }
        [string appendFormat:@"%@,",[modelUp toJSONString]];
    }
