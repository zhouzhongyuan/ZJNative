...
- (void)viewDidLoad {
    ...
    self.navigationItem.leftBarButtonItem =[self settNavi];
    ...
    if (arr.count>0) {
        UserModel *model = [arr firstObject];
        if (![_userJsonModel.user.lasttime isEqualToString:model.lasttime]) {
            [self saveUser];
            [self analysisGroup];
...
-(UIBarButtonItem *)settNavi{
    UIButton *backBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    backBtn.frame = CGRectMake(0, 0, 25, 25);
    [backBtn setImage:[UIImage imageNamed:@"ZJNativeBundle.bundle/arrow"] forState:UIControlStateNormal];
    //添加返回的方法
...
-(void)analysisStockDatabase{
    ...
    NSArray *arr1 = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (StockModel*p in arr1) {
...
-(void)analysisPriceDatabase{
    ...
    for (MaterialPriceModel*p in arr1) {
...
-(void)analysisType{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"PromotionTypeModel"];
    
    NSArray *arr1 = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (PromotionTypeModel*p in arr1) {
...
-(void)analysisRule{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"PromotionRuleModel"];
    
    NSArray *arr1 = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (PromotionRuleModel*p in arr1) {
...
-(void)analysisReason{
    ...
    for (RetreatReasonModel*p in arr1) {
...
-(void)analysisBrand{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"MaterialGroupModel"];
    
    NSArray *arr = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (MaterialGroupModel*p in arr) {
...
-(void)analysisGroup{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"CustomerGroupModel"];
    
    NSArray *arr = [kCoreDataContext executeFetchRequest:request error:nil];
    
    for (CustomerGroupModel*p in arr) {
...
-(void)analysisChannel{
    NSFetchRequest *request = [NSFetchRequest fetchRequestWithEntityName:@"DistributionChannelModel"];
